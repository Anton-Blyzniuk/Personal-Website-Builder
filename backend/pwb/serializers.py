...
class PortfolioItemWriteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PortfolioItem
        fields = ["title", "category", "description", "date", "order", "links"]
...
def _save_nested(pwb_unit, nested, replace=True):
    ...
    _handle("certifications", lambda data: (
        pwb_unit.certifications.all().delete(),
        Certification.objects.bulk_create([Certification(pwb_unit=pwb_unit, **d) for d in data]),
    ))
...
def _save_portfolio_items(pwb_unit, items_data):
    pwb_unit.portfolio_items.all().delete()
    items_to_create = []
    links_per_item = []
    for item_data in items_data:
        links_per_item.append(item_data.pop("links", []))
        items_to_create.append(PortfolioItem(pwb_unit=pwb_unit, **item_data))
    created = PortfolioItem.objects.bulk_create(items_to_create)
    all_links = [
        PortfolioItemLink(portfolio_item=item, **link_data)
        for item, links in zip(created, links_per_item)
        for link_data in links
    ]
    if all_links:
        PortfolioItemLink.objects.bulk_create(all_links)
